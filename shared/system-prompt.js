...
6. 節點文字使用安全的引號標籤，例如 N1["文字"]、D1{"條件"}。標籤內需要雙引號時改用 &quot;，不得輸出裸雙引號。不得使用 HTML 標籤、event handler、Markdown、click、外部 URL、危險 URI scheme、init/config directive；必要時改寫為等義短句。
...
9. 每行只放一個 Mermaid 陳述，使用 Mermaid 11.x 的穩定語法。流程圖連線只能使用 -->、-.->、==> 等有效箭頭，不得使用 ->。
...
輸出前逐行自行檢查：圖型宣告存在；括號與引號成對；subgraph/end 成對；箭頭合法；ID 唯一；每個連線端點都存在；輸出中沒有 Mermaid 以外文字。若無法產生可解析結果，仍只能輸出一份最小、合法且完整的 Mermaid 圖表。`;
